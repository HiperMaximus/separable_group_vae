...
where $$g, \tilde{g} \in G$$ and $$\mu$$ is the left-invariant Haar measure on $$G$$. This operation is equivariant to the left-regular representation of the group, i.e., $$\begin{aligned}(\mathcal{L}_{g'}f) \star_G k = \mathcal{L}_{g'}(f \star_G k)$$, where $$(\mathcal{L}_{g'}f)(g) = f(g'^{-1}g)\end{aligned}$$.
